...
class Ems
{
    ...
    public static function send($email, $code = null, $event = 'default')
    {
        ...
        $ems = \app\common\model\Ems::create(['title' => $title, 'event' => $event, 'email' => $email, 'code' => $code, 'ip' => $ip, 'createtime' => $time], true);
        if (!Hook::get('ems_send')) {
            ...
            Hook::add('ems_send', function ($params) {
                ...
                $result = $obj
                    ...
                    ->message("您的验证码是：" . $params->code . "，" . ceil(self::$expire / 60) . "分钟内有效。")
                    ->send();
                return $result;
            });
        }
